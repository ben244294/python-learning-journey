...
while True:
    task_list = input("Enter a list of tasks separated by commas")
    to_do_list = [task.strip() for task in task_list.split(
        ",")]  # .strip removes any accidental leading/trailing spaces,.strip converts a single input string into individual list items
    print(f"Your list of tasks are")
    for to_do in to_do_list:
        print(f" -  {to_do}")
    terminate = input("press x to quit or c to continue")
    if terminate == "x":
        break
